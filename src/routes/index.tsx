    ...
    <>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <img src={heroImg} alt="Caffè Delle Commari interior" width={1920} height={1280} className="h-full w-full object-cover" />
        ...
        <div className="relative mx-auto flex min-h-[90vh] max-w-6xl flex-col justify-center px-5 py-24 lg:px-8">
          <span className="inline-flex w-fit items-center gap-2 rounded-full border border-gold/40 bg-charcoal/40 px-4 py-1.5 text-xs font-medium uppercase tracking-widest text-gold backdrop-blur">
            <span className="h-1.5 w-1.5 rounded-full bg-gold" /> Near the Vatican · Roma
          </span>

          <h1 className="mt-6 font-display text-5xl font-bold leading-[1.05] text-cream text-balance sm:text-6xl md:text-7xl lg:text-8xl">
            Caffè Delle <span className="italic text-gold">Commari</span>
          </h1>
          <p className="mt-5 max-w-xl text-lg text-cream/85 md:text-xl">{SITE.tagline}</p>

          <div className="mt-10 flex flex-wrap items-center gap-4">
            <Link
              to="/reserve"
              className="rounded-full bg-primary px-8 py-3.5 text-base font-semibold text-primary-foreground shadow-warm transition-transform hover:scale-[1.03]"
            >
              Reserve a Table
            </Link>
            <Link
              to="/menu"
              className="rounded-full border border-cream/30 px-8 py-3.5 text-base font-semibold text-cream backdrop-blur-sm transition hover:bg-cream/10"
            >
              View Menu
            </Link>
          </div>

          <div className="mt-12 flex flex-wrap gap-x-6 gap-y-3 text-sm text-cream/85">
      ...
      <section className="mx-auto max-w-6xl px-5 pb-24 lg:px-8">
        <div className="grid gap-6 md:grid-cols-3">
          {features.map((f) => (
            <article
              key={f.title}
              className="group rounded-2xl border border-border bg-card p-8 shadow-soft transition hover:-translate-y-1 hover:shadow-warm"
            >
      ...
      <section className="bg-secondary/60 py-24">
        <div className="mx-auto grid max-w-6xl gap-12 px-5 md:grid-cols-2 md:items-center lg:px-8">
          ...
          <div>
            ...
            <Link
              to="/menu"
              className="mt-10 inline-flex rounded-full bg-charcoal px-7 py-3 text-sm font-semibold text-cream transition hover:bg-primary"
            >
      ...
      <section className="mx-auto max-w-5xl px-5 py-24 text-center lg:px-8">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-3">
          {team.map((n) => (
            <span key={n} className="rounded-full bg-gradient-gold px-6 py-2 text-sm font-semibold text-gold-foreground shadow-soft">
              {n}
            </span>
          ))}
        </div>
      ...
      {/* CTA */}
      <section className="mx-auto max-w-6xl px-5 pb-24 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-warm p-12 text-center shadow-warm md:p-20">
          ...
          <div className="relative">
            ...
            <Link
              to="/reserve"
              className="mt-8 inline-flex rounded-full bg-cream px-8 py-3.5 text-base font-semibold text-charcoal transition-transform hover:scale-[1.03]"
            >
              Reserve a Table
            </Link>
          </div>
